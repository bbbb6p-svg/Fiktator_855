...
function isPatternNumber(num) {
  ...
  // 1. تكرار رقم واحد 5 مرات او اكثر: 77777
  if (/(.)\1{4,}/.test(s)) return true;
  // 2. تكرار رقمين: 77778888
  if (/(\d{2})\1{1,}/.test(s)) return true;
  // 3. تكرار 3 ارقام: 777888777
  if (/(\d{3})\1{1,}/.test(s)) return true;
  // 4. متسلسل
  if (/01234|12345|23456|34567|45678|56789/.test(s)) return true;
...
function scorePattern(num) {
  ...
  // 1. نقاط على طول التكرار: كل رقم متكرر = 15 نقطة
  const repeats = s.match(/(.)\1+/g) || [];
  for (const r of repeats) score += r.length * 15;

  // 2. بونص لو الرقم كله نفس الشي: 7777
  if (/^(.)\1+$/.test(s)) score += 200;

  // 3. بونص للمتناظر المزدوج: 77778888
  if (/(\d{2})\1{1,}/.test(s)) score += 120;

  // 4. بونص للمتسلسل
  if (/01234|12345|23456|34567|45678|56789/.test(s)) score += 50;
...
async function handleNumberCommand(sock, msg, cmd, args) {
  ...
  if (cmd === 'مميز') {
    ...
    for (const n of allNumbers) {
      if (isPatternNumber(n)) {
        const s = n.replace(/\D/g, '');
        let multiplier = 1;

        // لو كله نفس الرقم نضرب في طوله. 7777 = 7
        if (/^(.)\1+$/.test(s)) multiplier = s.length;
        // لو متناظر مزدوج نضرب في 5
        else if (/(\d{2})\1{1,}/.test(s)) multiplier = 5;

        map.set(n, (map.get(n) || 0) + multiplier);
      }
    }

    const data = [...map.entries()]
     .map(([num, count]) => ({ num, count, score: scorePattern(num) }))
     .sort((a, b) => b.score - a.score || b.count - a.count); // ترتيب بالنقاط ثم التكرار
    ...
    if(txt.length > 4000){
      const parts = txt.match(/[\s\S]{1,4000}/g);
      for(const part of parts){
        await sock.sendMessage(jid, { text: part }, { quoted: msg });
      }
    } else {
